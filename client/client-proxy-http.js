...
async function main(log) {
		...
		const server = net.createServer({
			...
			try {
				...
				if (res1.status !== '200 OK')
					log.warn(getNow(), port, sv, svc, 'L[2030] conn.status:', res1.status);
				...
				const locConn = {
					...
					writeLocal(seq, data, onErr) {
						this.sends[seq] = () => {
							...
							log.trace(getNow(), port, sv, 'wrlc:', svc, cID, 'r#:', seq, 'writeLocal');
						};
						this.flushLocal();
					},
					endLocal(seq) {
						this.sends[seq] = () => {
							log.trace(getNow(), port, sv, 'edlc:', svc, cID, 'r#:', seq, 'endLocal', !!this.socket);
					...
					if (!dataTimer)
						dataTimer = setTimeout(async () => {
							try {
								...
								// log.warn(getNow(), port, COLOR_MAGENTA + sv, 'data:', svc, 'L[3000] ' + cID + COLOR_RESET, 'size:', dataList.length);
								...
								log.trace(getNow(), port, COLOR_MAGENTA + sv, 'data:', svc, 'L[3000] ' + cID + COLOR_RESET);
								// L[3010]
								const res = await rpc(agent, port, 'POST', 'snd1',
									{ x: 'L[3010]', sv, port, svc, cID, locSeq: locConn.locSeq++ }, data);
								if (res.status !== '200 OK')
									log.warn(getNow(), port, sv, svc, ...redError('conn.snd1.sts: ' + res.status));
							} catch (err) {
				...
				soc.on('error', async (err) => {
					try {
						log.warn(getNow(), port, ...redError(sv + ' err1: ' + svc + ' L[soc.err]:'), ...redError(err));
						// L[err.xxxx]
						const res = await rpc(agent, port, 'GET', 'end1',
							{ x: 'L[err.xxxx]', sv, port, svc, cID, locSeq: locConn.locSeq++ });
						if (res.status !== '200 OK')
							log.warn(getNow(), port, sv, svc, ...redError('soc.err.end.sts: ' + res.status));
						release('soc.err:', ...redError(err), cID);
					} catch (err) {
						release('soc.err.err:', ...redError(err), cID);
					}
				});
				soc.on('end', async () => {
					try {
						log.warn(getNow(), port, COLOR_MAGENTA + sv, 'end1:', svc, cID + COLOR_RESET);
						// L[end1.xxxx]
						const res = await rpc(agent, port, 'GET', 'end1',
							{ x: 'L[end1.xxxx]', sv, port, svc, cID, locSeq: locConn.locSeq++ });
						if (res.status !== '200 OK')
							log.warn(getNow(), port, sv, 'end1:', svc, ...redError('end1.sts: ' + res.status));
						release('soc.end:', cID);
					} catch (err) {
						release('soc.end.err:', ...redError(err), cID);
					}
				});
			} catch (err) {
			...
			function release(...args) {
				...
				log.warn(getNow(), port, COLOR_MAGENTA + sv, 'rels:', svc, '[release]', ...args, COLOR_RESET);
				locConn && locConn.endLocal(locConn.remSeq);
				localConnections.delete(cID);
			}
		});
		server.on('error', err => // L[1090] server.error
			log.trace(getNow(), port, sv, 'svrx:', svc, 'L[1090] svr.err:', ...redError(err)));
		server.listen(port, () => // L[1020] server.listen
			log.trace(getNow(), port, sv, 'svrx:', svc, 'L[1020] svr.listen'));
	});
				...
				try {
					...
					if (cmd === 'conn') { // R[2110] conn
						...
						const soc = net.connect({ host, port }, async () => {
							// log.trace(dt, threadId, locSv, ...redError('[2200] con1:'), hostPort);
							// R[2200] con1
							try {
								const res = await rpc(agent, threadId, 'GET', 'con1',
									{ x: 'R[2200]', sv, svID, svc, cID });
								if (res.status !== '200 OK')
									log.warn(dt, threadId, locSv, ...redError('R[2200] con1.sts: ' + res.status));
							} catch (err) {
								// TODO
								log.warn(dt, threadId, locSv, 'R[2200] con1.err: ', ...redError(err));
							}
						});
						// R[2130] conn
						remoteConnections.set(cID, {
							...
							writeRemote(seq, data, onErr) {
								this.sends[seq] = () => {
									this.socket.write(data, onErr);
									log.trace(getNow(), threadId, 'wrrm:', locSv, cID, 'l#:', seq, 'writeRemote');
								};
								this.flushRemote();
							},
							endRemote(seq) {
								log.trace(getNow(), threadId, 'edrm:', locSv, cID, 'l#:', seq, 'endRemote', !!this.socket);
								if (this.socket) this.socket.end();
								// @ts-ignore
								this.socket = null;
							},
						...
						soc.on('data', async (data) => { // R[3200] snd6
							...
							if (!dataTimer)
								dataTimer = setTimeout(async () => {
									try {
										stdout.write(`\r\nTotal[${dataLength.toLocaleString()}]\r\n`)
										// log.warn(getNow(), threadId, COLOR_MAGENTA + locSv, 'snd6: R[3200] ' + cID + COLOR_RESET, 'size:', dataList.length);
										...
										if (res.status !== '200 OK')
											log.warn(dt, threadId, ...redError(locSv + ' snd6: R[3200] sts: ' + res.status));
									} catch (err) {
										// TODO
										log.warn(dt, threadId, ...redError(locSv + ' snd6: R[3200] err: '), ...redError(err));
									}
								}, DATA_TIMEOUT);
						});
						soc.on('error', async (err) => { // R[err6] err6.xxxx R[xxxx]
							log.warn(dt, threadId, ...redError(locSv + ' err6: ' + cID), ...redError(err));
							try {
								const res = await rpc(agent, threadId, 'GET', 'end6',
									{ x: 'R[err6]', sv, svID, svc, cID, remSeq: remoteConnections.get(cID).remSeq++ });
								if (res.status !== '200 OK')
									log.warn(dt, threadId, ...redError(locSv + ' err6: ' + cID + ' sts: ' + res.status));
							} catch (err) {
								// TODO
								log.warn(dt, threadId, ...redError(locSv + ' err6: ' + cID + ' err:'), ...redError(err));
							}
						});
						soc.on('end', async () => { // R[end6.xxxx] end6 R[xxxx]
							try {
								...
								if (res.status !== '200 OK')
									log.warn(dt, threadId, ...redError(locSv + ' end6: sts: ' + res.status));
							} catch (err) {
								// TODO
								log.warn(dt, threadId, ...redError(locSv + 'end6: err:'), ...redError(err));
							}
						});
					}
					else if (cmd === 'con3') { // L[2230.xxxx] con3
						const locConn = localConnections.get(cID);
						log.trace(dt, threadId, locSv, 'con3:', cID);
						...
						log.trace(dt, threadId, COLOR_CYAN + locSv, 'init: X[0140]',
						...
						try {
							if (!remConn || !remConn.socket) {
								log.warn(dt, threadId, locSv, ...redError('snd4: R[3040] snd1.err:'), ...redError('remConn.socket is null'));
								try {
									const res1 = await rpc(agent, threadId, 'GET', 'end6',
										{ x: 'R[end6.zzzz]', sv, svID, svc, cID });
									if (res1.status !== '200 OK')
										log.warn(dt, threadId, ...redError(locSv + ' end6: zzzz sts: ' + res1.status));
								} catch (err) {
									// TODO
									log.warn(dt, threadId, ...redError(locSv + 'end6: zzzz err:'), ...redError(err));
								}
							}
							else {
								try {
									...
									if (err.code === 'EPIPE')
										log.warn(dt, threadId, locSv, ...redError('snd4: R[3042] snd1.err: ' + err));
									else
										log.warn(dt, threadId, locSv, ...redError('snd4: R[3043] snd1.err:'),
											...redError(err));
									// TODO
									...
									try {
										...
										if (res.status !== '200 OK')
											log.warn(dt, threadId, ...redError(locSv + ' end6: yyyy sts: ' + res.status));
									} catch (err) {
										// TODO
										log.warn(dt, threadId, ...redError(locSv + 'end6: yyyy err:'), ...redError(err));
									}
								}
							}
							...
							if (res.status !== '200 OK')
								log.warn(dt, threadId, locSv, ...redError('snd2: R[3050] snd2.sts: ' + res.status));
						} catch (err) {
							// TODO
							log.warn(dt, threadId, locSv, ...redError('snd2: R[3050] snd2.err:'), ...redError(err));
						}
					}
						...
						if (locConn && locConn.socket)
							locConn.writeLocal(remSeq, res.body,
								err => err && log.warn(dt, threadId, locSv, 'snd8:', cID, 'L[3230]', ...redError(err)));
						else
							log.warn(dt, threadId, locSv, 'snd8:', cID, 'L[3230]', ...redError('locConn.socket is null'));
					}
					else if (cmd === 'end1') { // R[end1.xxxx] end1
						const { locSeq } = res.options;
						remConn && remConn.endRemote(locSeq) ||
							log.debug(dt, threadId, COLOR_MAGENTA + locSv, 'end1:', cID, 'remConn.socket closed' + COLOR_RESET);
					}
					else if (cmd === 'end6') { // R[end6.xxxx] end6
						const locConn = localConnections.get(cID);
						const { remSeq } = res.options;
						locConn && locConn.endLocal(remSeq) ||
							log.debug(dt, threadId, COLOR_MAGENTA + locSv, 'end6:', cID, 'locConn.socket closed' + COLOR_RESET);
					}
						...
						log.fatal(dt, threadId, ...redError(locSv + ' recv: cmd.err: \"' + cmd + '\"'));
						agent.destroy();
					...
					log.warn(getNow(), threadId, ...redError(locSv + ' recv: err:'),
	...
	async function rpc(agent, num, method, cmd, args, body = undefined) {
		log.trace(getNow(), num, COLOR_GREEN + locSv, cmd + ': ' + myStringify(args) + COLOR_RESET);
		if (body && method !== 'POST')
		...
		// 	log.trace(dt, num, locSv, 'rpc.res', COLOR_CYAN + res.rawHeaders[i] + ': ' + res.rawHeaders[i + 1] + COLOR_RESET);
		// log.trace(dt, num, locSv, 'rpc.res', COLOR_CYAN + 'body [' + res.body.toString() + ']' + COLOR_RESET);
